...
import { startSession, verifyPassword } from '@growth-os/authn';
import {
  afterFailedAttempt,
  CLEARED_LOCKOUT,
  isLocked,
...
export async function signIn(deps: SignInDependencies, input: SignInInput): Promise<SignInResult> {
  return await deps.unitOfWork.transaction(
    'identity sign-in',
    async (repositories) => await signInInTransaction(bindToTransaction(deps, repositories), input),
  );
}

async function signInInTransaction(
  deps: SignInDependencies,
  input: SignInInput,
